let cartItemS3=JSON.parse(localStorage.getItem("cartItemS3"))||[];
function addToCart(productCard){
    ...
    const existingItem=cartItemS3.find((item)=>item.name ===name);
    if(existingItem){
...
{
    cartItemS3=cartItemS3.filter((item) =>item.name !==name);
    updateLocalStorage();
    if(document.getElementById("cartItemS3"))
    {
        displayCartItems();
    }
}
function updateCartCount()
{
    ...
    const itemCount=cartItemS3.reduce((count,item) => count +item.quantity,0);
    if(countElement)
    {
...
function displayCartItems(){
    const cartContainer=document.getElementById("cartItemS3");
    ...
    cartItemS3.forEach((item)=>{
        ...
        total +=itemTotal;
        const cartItemS3=document.createElement("div");
        cartItemS3.className="cart-item";
        cartItemS3.innerHTML=`
       ...
       <input type="text" name="" class="cart-item-quantity" value="${item.quantity}" min="1" onchange="updateQuantity('${item.name}',this.value)">
        ...
        `;
        cartContainer.appendChild(cartItemS3);
    });
    if(totalElement)
    {
    ...
    function changeQuantity(name,delta){
        const item=cartItemS3.find((item)=>item.name ===name);
        if(item){
            item.quantity +=delta;
            if(item.quantity <1){
                removeItem(name);
